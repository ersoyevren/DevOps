  ...
  template: 
    ...
    spec:
      ...
      - name: web-flask-pod
        ...
        - containerPort: 5000
```
# `minReadySeconds`, podun hazir olmasi icin beklenilecek sure.
# `deployment strategy` olarak iki tur vardir.`rollingUpdate` ile kac tane silip kac tane ekliyecegiz. bunu yuzde olarak da yapabiliriz. default olarak yuzde geliyor.`recreate` ile ise butun podlari silip sonra olusturuyor.cok onerilmiyor.
- Create the web-flask Deployment.
  
```bash
kubectl apply -f web-flask.yaml
#-f . dersek dosyanin altinda bulunan tum dosyalari calistir demek. . yerine isim yazarsak ismi belirtilen yaml dosyasini calistirir.
```
...
```
## imagePullPolicy, `IfNotPresent`(eger localde yoksa pull et), `Always`(daima remote pull et), `Never`(localde varsa pull et yoksa calistirma) degerlerini alir. 

## restartPolicy, container durursa `Always`(daima yeniden calistir), `OnFailure`(eger hata sonucu durmussa calistir), `Never`(ne sebeple durmus olursa olsun calistirma) degerlerini alir. 
...
/ # ping 172.16.166.180
#exec -it ile dosyanin icine giriyor.
#exec -d ile arkada calistiriyor.
#exec ile tek komut calistiriyor. 
  # kubectl exec forcurl --ls `gibi`
```
...
```
# soru curl burada ne ise yariyor.
...
spec:
  ...
  selector:
    env: front-end 
```
# NodePort 30000-32767 arasinda degisiyor. eger biz belirlemezsek otomatik ataniyor. 
